...
<body>
	...
	<script>
		function fetchDataAndUpdateCharts() {
			$.ajax({
				...
				success : function(responseData) {
					if (responseData.daily
							&& responseData.daily.data.length > 0) {
						updateDailyChart(responseData.daily);
					} else {
						console.error('Daily data is empty or invalid.');
					}
					if (responseData.weekly
							&& responseData.weekly.data.length > 0) {
						updateWeeklyChart(responseData.weekly);
					} else {
						console.error('Weekly data is empty or invalid.');
					}
					if (responseData.monthly
							&& responseData.monthly.data.length > 0) {
		...
		function updateDailyChart(data) {
			var dailyCtx = document.getElementById('dailyChart').getContext(
					'2d');
			new Chart(dailyCtx, {
				type : 'pie',
				data : {
		...
		function updateWeeklyChart(data) {
			var weeklyCtx = document.getElementById('weeklyChart').getContext(
					'2d');
			new Chart(weeklyCtx, {
				type : 'bar',
				data : {
