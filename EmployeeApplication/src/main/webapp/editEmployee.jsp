	...
	<div class="container">
		...
		<form action="ManageEmployeeServlet" method="post">
			<input type="hidden" name="action" value="update"> <label
				for="id">Employee ID:</label> <input type="text" id="id" name="id"
				value="<%=user.getEmpId()%>" readonly> <label for="name">Name:</label>
...
<select id="role" name="role" required style="padding: 10px; margin: 10px 0; border-radius: 5px; border: 1px solid #ccc;">
    <option value="Employee" <%= "Employee".equals(user.getRole()) ? "selected" : "" %>>Employee</option>
		...
		<div class="center">
			<a href="manageEmployees.jsp" class="back-to-dashboard">Back to
				Manage Employees</a>
		</div>
	</div>
</body>
